...
def declare_emergency(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Prepare data for dispatcher (matching dispatcher schemas.py)
            alert_data = {
                'user_id': request.user.id,
                'emergency_type': form.cleaned_data['emergency_type'],
                'description': form.cleaned_data['description'],
                'location': form.cleaned_data['location'],  # Send as string as expected by dispatcher
            }
            ...
            if success and alert_id:
                # Create Alert record in local DB for tracking
                ...
                # Store alert ID in session for reference if needed, but we'll redirect using the ID directly
                request.session['last_alert_id'] = alert.id
                ...
                return redirect('alert_confirmation')
            else:
...
def status_check(request, report_id=None):
    """Public status check page for a specific report or alert."""
    ...
    # Try to find EmergencyReport by UUID first
    if report_id:
        try:
            report = EmergencyReport.objects.get(id=report_id)
        except (EmergencyReport.DoesNotExist, ValueError):
            # If not found or invalid UUID, try to find Alert by ID
            try:
                alert = Alert.objects.get(id=int(report_id))
    ...
    # If we have an alert, get its responses
    responses = None
    if alert:
        responses = Response.objects.filter(alert=alert).select_related('response_unit')
...
def report_add(request):
    """Admin view to manually add a report."""
    if not request.user.profile.is_admin():
        messages.error(request, 'Access denied.')
        return redirect('home')
    
    from .forms import AdminEmergencyReportForm
    
    if request.method == 'POST':
        form = AdminEmergencyReportForm(request.POST)
        if form.is_valid():
            report = form.save()
            messages.success(request, f'Emergency report {report.id} created successfully.')
            return redirect('dashboard')
    else:
        form = AdminEmergencyReportForm()
    
    return render(request, 'report_form.html', {
        'form': form,
        'title': 'Add New Emergency Report',
...
def report_edit(request, report_id):
    ...
    if request.method == 'POST':
        form = AdminEmergencyReportForm(request.POST, instance=report)
        if form.is_valid():
            form.save()
            messages.success(request, f'Emergency report {report.id} updated successfully.')
            return redirect('dashboard')
    else:
        form = AdminEmergencyReportForm(instance=report)
    
    return render(request, 'report_form.html', {
        'form': form,
        'report': report,
        'title': f'Edit Emergency Report #{str(report.id)[:8]}',
        'button_text': 'Update Report'
    })
...
def report_delete(request, report_id):
    ...
    # If HTMX request, we can just return empty response or something to remove the row
    if request.headers.get('HX-Request'):
        return JsonResponse({'success': True, 'message': f'Report {report_id_str} deleted.'})
    
    messages.success(request, f'Emergency report {report_id_str} deleted successfully.')
    return redirect('dashboard')
...
def alert_confirmation(request):
    """Display alert confirmation page with alert ID and status details."""
    from .models import Alert, Response
    
    # Get alert ID from session (using .get() to not clear it immediately if user refreshes)
    alert_id = request.session.get('last_alert_id')
...
def my_alerts(request):
    ...
    for alert in alerts:
        responses = Response.objects.filter(alert=alert).select_related('response_unit')
        alerts_with_responses.append({
            'alert': alert,
            'responses': responses
        })
    
    context = {
        'alerts_with_responses': alerts_with_responses,
    }
